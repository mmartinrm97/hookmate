...
3. **Construct abstractions**: CDK `DatabaseInstance` composes many sub-resources. In Terraform each component is explicit: `aws_db_instance`, `aws_db_subnet_group`, `aws_db_parameter_group`
...
6. **Cost visibility**: Infracost runs on every Terraform PR and posts an estimated monthly cost delta as a PR comment. CDK does not have an equivalent first-class cost diff tool — this is a deliberate demonstration of FDE cost awareness practices
